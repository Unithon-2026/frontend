...
지도가 켜졌다 꺼졌다 하는 것처럼 보이던 원인들을 이렇게 막아 뒀습니다.

| 상황 | 처리 |
| --- | --- |
| 보이는 영역에 매장이 0건 | 안내를 지도 위 **작은 카드**로만 띄웁니다. 예전처럼 화면 전체를 덮지 않습니다. |
| 첫 화면·필터 변경 결과가 화면 밖 | 데이터가 있는 곳으로 **한 번만** 자동으로 화면을 맞춥니다(`setBounds`). |
| 드래그 한 번에 `idle` 연타 | 220ms 로 묶어 마지막 한 번만 조회합니다. |
| 지도 이동마다 핀 재생성 | 오버레이를 `shopId` 기준으로 **재사용**하고, 화면 밖으로 나간 것만 지웁니다. |
| `/shops/map` 조회 실패 | 0건으로 취급하지 않고 **직전 핀을 유지**한 채 실패 안내만 띄웁니다. |
| 다른 화면에 갔다 돌아옴 / 창 크기 변경 | 컨테이너가 `display:none` 이었으므로 `relayout()` 후 중심을 복원합니다. |
| **새로고침 직후 타일이 한 조각만 그려짐** | 컨테이너 크기가 잡힌 뒤에 지도를 만들고(`waitForSize`), 이후 크기 변화는 `ResizeObserver` 가 받아 `relayout()` 합니다. |
| 사용자가 지도를 직접 드래그·확대 | 대기 중이던 자동 맞춤을 **취소**합니다. 안 그러면 핀 있는 곳으로 옮기는 도중 지도가 튕겨서 숫자가 사라진 것처럼 보입니다. |

### 국내 밖으로 나가지 않게

카카오 지도는 **국내 타일만** 제공합니다. 밖으로 나가면 회색 여백만 남아 지도가 잘린 것처럼 보이므로 이렇게 막습니다.

- `setMaxLevel(13)` — 더 축소하면 한반도가 점이 되고 주변이 전부 빕니다.
- 보이는 영역이 `KOREA_BOUNDS`(위 33.0–38.7 / 경 124.5–132.0, 제주·울릉·독도 포함)를 벗어나면 중심을 밀어 넣습니다. **중심이 아니라 화면 전체** 기준이라 가장자리에 여백이 남지 않습니다. 현재 배율이 한국보다 넓으면 한반도 한가운데로 보냅니다.
- `currentBbox()` 는 뒤집힌 좌표를 바로잡고 국내 범위로 잘라서 질의합니다. 크게 축소했을 때 `getBounds()` 가 이상한 값을 주면 "아무것도 없는" 질의가 되어 핀이 통째로 사라지던 문제를 막습니다.

### 지도 위 요소 배치

| 위치 | 요소 |
| --- | --- |
| 좌상단 | 등급 범례, 그 아래로 안내 문구(`#mapnote`)가 쌓임 |
| 우상단 | 표시 중인 매장 수 (`N개 표시`) |
| 하단 가운데 | 빈 상태 카드 (지도를 덮지 않음) |

좌상단 묶음은 `max-width:min(340px, calc(100% - 150px))` 라 우상단 카운트와 겹치지 않습니다.
