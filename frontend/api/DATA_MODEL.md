...
서버에 아무것도 남기지 않습니다. 관심 저장·메모·영업 상태는 브라우저 `localStorage`
(`meetroute.watchlist.v1`) 에만 쌓이므로 **다른 기기·팀원과 공유되지 않고**, 서버에 엔터티가
생기면 그대로 마이그레이션 대상이 됩니다. 팀 현황·데이터 관리 수치는 하드코딩된 더미입니다.
